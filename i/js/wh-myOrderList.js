...
function parseViewWithData(data) {
    ...
    if ((data.length <= 0) && (page == 1)) {
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            if (element.buttons) {
                for (let j = 0; j < element.buttons.length; j++) {
                    ...
                    appendText += '<span class="am-fr wh-buttonCustomer am-margin-right-xs  am-text-danger" price='+ element.amount +' id="' + element.id + '"  value="' + button.type + '">' + button.text + '</span>';
                }
            }
            appendText += '</div>';
            appendText += '</td>';
        ...
        $("tbody").click(function() {
            var orderId = $(this).attr('id');
            window.location.href = baseURL + "wxpages/classify/wh-orderDetail.html?orderId=" + orderId;
        })
    }
}
//根据按钮类型判断跳转事件
