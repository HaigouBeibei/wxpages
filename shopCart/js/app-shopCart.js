...
function initSettleButton() {
    $('#settleButton').off('click').click(function() {
        ...
        if (myAddress.list.length != 0) {
            ...
            window.location.href = '../classify/orderPreview.html?type=cart' + '&deliveryAddressId=' + addressID;
        } else {
            console.log('2')
            $('#my-actions').modal('close');
            $('#address-prompt').modal({
                onConfirm: function(e) {
                    window.location.href = "../classify/wxpages/i/wh-myAddress.html?callback=" + '//////' + baseURL + "/wxpages/shopCart/index.html" + '////////';
                }
            });
        }
    });
